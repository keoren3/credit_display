...
def validate_date(xl_date, date_mode):
    ...
    if xl_date.ctype == 3:
        ...
        updated_date = '/'.join(updated_date.split('/')
                                [:-1]) + '/' + correct_year
    return updated_date
...
def get_data_from_row(row, sheet_type, date_mode):
    logger.debug(
        "Checking if a transaction was made in the row: '{0}'".format(row))

    if '.' not in str(row[2].value):
        logger.debug("No transaction was made, continuing...")
        return None

    date = validate_date(row[0], date_mode)
    if sheet_type == 'visa':
        business_name, deal_value, charge_value, more_details = [
            cell.value for cell in row[1:]]
    if sheet_type == 'mastercard':
        business_name, deal_value, _, charge_value, _, more_details = [
            cell.value for cell in row[1:-1]]
...
def get_transactions(work_sheet, sheet_type, date_mode):
    transactions = []
    logger.info("Going over all rows in sheet")
    ...
    for i in range(work_sheet.nrows):
        ...
        if curr_deal:
            logger.debug(
                "Adding deal '{0}' to transactions".format(curr_deal))
            transactions.append(curr_deal)

    logger.debug("Returning transactions: {0}".format(transactions))
    return transactions
...
def get_sheet_type(work_sheet):
    ...
    if 'מסטרקארד' in work_sheet.row(3)[0].value:
        return 'mastercard'
    raise KeyError(
        "ERROR: Work sheet {0} is not recognized!".format(work_sheet))


def fix_corrupt_excel(sheet_name):
    logger.info("Re-creating excel sheet to fix corruption")

    sheet_file = io.open(sheet_name, "r", encoding="utf-16")
    sheet_data = sheet_file.readlines()
    excel_doc = Workbook()
    sheet = excel_doc.add_sheet("FixedSheet_{0}".format(
    ...
    logger.info("Corruption fixed! Continuing...")
...
def get_data_from_excel(sheet_name):
    ...
    try:
        logger.info(
            "Getting transactions from excel sheet: '{0}'".format(sheet_name))

        workbook = xlrd.open_workbook(sheet_name)
    except xlrd.XLRDError as e:

        logger.warning("Excel file {0} was corrupted. Exception: '{1}'.".format(
            sheet_name, e))
    ...
    logger.info("First sheet name: '%s'" % work_sheet.name)
